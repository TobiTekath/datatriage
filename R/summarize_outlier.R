...
#' @param sort Sort the outlier values? Can be FALSE for no sorting, TRUE or "descending" for descending order, or "ascending" for ascending order.
...
summarize_outlier <- function(vec, minimal_unique_values = 10, method = c("iqr", "zscore_robust"), threshold = list(zscore_robust = 7, iqr = 5), sort = TRUE) {
  ...
  checkmate::assert(
    checkmate::check_flag(sort),
    checkmate::check_choice(sort, choices = c("ascending", "descending"), null.ok = FALSE),
    combine = "or"
  )
  ...
  if (is.numeric(vec)) {
    if (length(unique(vec)) >= minimal_unique_values) {
      ...
      if (length(which(outlier)) > 0) {
        ...
        return(stringr::str_flatten_comma(out))
      }
    }
  }
  return("")
